...
- [x] For **≥10** mainnet addresses, local verified `eth_getBalance` matches an **independent** differential oracle (≠ sole proof/header upstream). Live 2026-08-19: **19 unique / 214 compared / 0 mismatch** (Ankr proofs, BlastAPI oracle) over a **≥1h** re-diff soak. The **≥24 h soak passed 2026-08-24** (24.06 h, exit 0).
...
| C | **Near-term work:** keep milestones honest — Demo Slice closed; the **≥24h soak passed 2026-08-24** (24.06 h, exit 0) and a 4 h soak on the shipped build followed; **Pasteur went live 2026-08-25** and needed no client change. MVP-2 `eth_call` / `eth_estimateGas` / local `BLOCKHASH` in tree; **FF landed** (verified live 2026-08-21) and block tags now resolve to the BLS-finalized head **by default** (2026-08-25), `--finality confirmation-depth` pins the old rule. Live Parlia pins: T=**8**, Safe lag ~106–112 on the fallback path and **2** on the default, proof window 112. `--allow-unsafe-head-reads` not implemented. |
...
1. **Freeze hardfork table** from a pinned `bnb-chain/bsc` commit (epochLength, turnLength, `extraData` versions for Maxwell/Fermi/…). **ANSWERED:** frozen at `bnb-chain/bsc` v1.7.8 `cdb7548b5baa…` in [hardfork-table.md](./hardfork-table.md), with `upstream-pin.yml` diffing the four transcribed files weekly. Pasteur (2026-08-25) moved none of them.
...
6. **Receipt verification priority vs `eth_call` — ANSWERED:** both shipped. `eth_call` landed first (MVP-2 slice 1); receipts are now bound to the sealed `receiptsRoot`, and `eth_getLogs` is built from them a block at a time, over ranges up to 128 blocks.
