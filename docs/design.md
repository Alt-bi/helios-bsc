...
6. **Receipt verification priority vs `eth_call` — ANSWERED:** both shipped. `eth_call` landed first (MVP-2 slice 1); receipts are now bound to the sealed `receiptsRoot`, and single-block `eth_getLogs` with them.
