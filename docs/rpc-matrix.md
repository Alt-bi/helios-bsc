...
Remaining (later; **not done**): `receiptsRoot` proofs; serving `transactionsRoot`-bound tx hashes in `eth_getBlock*` (ordered trie helper is in tree; RPC still `transactions: []`). `accessList` prefetch is **closed**. Historical `n≤Safe` state reads and the proof-round seed budget are **closed**.
