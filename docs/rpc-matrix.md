...
| `eth_call` | Verified (constrained) | Local verified header `n ≤ Safe` (`latest`→Safe). Requires `to` + optional `data`; **no create**, **no state overrides**. `accessList` prefetch **not implemented** at this commit (`95e0e6c`; may land separately). Seed is to/from/coinbase only and does not consume `MAX_PROOF_ROUNDS`. Proofs via `eth_getProof` at the requested **hash/number**; execute in **revm**. Unproven SLOAD/account → `-32001` (fail-closed, not zero). `BLOCKHASH` from locally verified headers (`n ≤` executing block in the 256-window); in-window unknown → `-32001`; out-of-window/current → `0` (never `eth_getBlock` from untrusted RPC). Gas cap **50_000_000**; max **8** proof rounds; calldata ≤ **128 KiB**; at most **32** accounts. Revert / Halt → JSON-RPC **code 3** (geth), not `-32001`. Never proxied to upstream `eth_call`. EIP-1898 object block ids → `-32602`. |
| `eth_getBlockByNumber` | Header-verified | At or below Safe; `fullTx=true` → `-32601`; txs always `[]`. `transactionsRoot` is the sealed header field (Hash()-bound); tx-trie hashes **not implemented** at `95e0e6c` (may land separately). Served from the locally stored sealed header when present (no re-fetch); otherwise re-fetch must `Header.Hash()` to the local verified hash |
| `eth_getBlockByHash` | Header-verified | Hash in local chain and `number ≤ Safe`. Same stored-header / Hash() bind. `transactionsRoot` hashes **not implemented** (txs `[]`; no tx-trie). |
...
| `eth_estimateGas` | Verified (constrained, best-effort) | Same policy as `eth_call` (`n ≤ Safe`; `to` + optional `data`; **no create**; **no state overrides**; no `blobVersionedHashes` / `authorizationList`; calldata ≤ **128 KiB**; at most **32** accounts). `accessList` prefetch **not implemented** at this commit. Never proxied to upstream `eth_estimateGas` / `eth_call`. Unproven SLOAD/account → `-32001`. `BLOCKHASH` from local verified headers (same as `eth_call`). Revert / Halt → JSON-RPC **code 3**. Binary search `TX_GAS..=min(user gas, 50_000_000, block.gasLimit)` (geth/reth; not a single `gas_used`). Max **8** proof rounds for the whole estimate. Gas is **not consensus**. |
...
Remaining (later; **not done** at `95e0e6c`; may land separately): `receiptsRoot` proofs; `transactionsRoot` hashes in `eth_getBlock*` (no tx-trie; txs stay `[]`); `accessList` prefetch (**not implemented**). Historical `n≤Safe` state reads and the proof-round seed budget are **closed** (seed does not consume `MAX_PROOF_ROUNDS`).
