...
- **Maxwell FF recents prune** depends on Fast Finality / BLS. That dependency is now **satisfied** — the snapshot tracks a BLS-finalized head — so this item is unblocked and is the natural next piece of consensus work.
...
5. Fast Finality BLS — **implemented and verified** (see the row above). Two follow-ups remain: serve the `finalized` tag from BLS finality (a wallet-visible behavioural change — soak it first), and the Maxwell FF `recents` prune, now unblocked. Because BLS finality lands **2 blocks** behind the tip instead of 106–112, the `eth_getProof` provider window stops being the binding constraint it is in [docs/proof-provider-matrix.md](docs/proof-provider-matrix.md) — but only once the tag actually resolves there.
