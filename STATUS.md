...
6. Optional remaining (later; **not done**): `receiptsRoot` RPC proofs on mined receipts/logs; `eth_getLogs`. Execution `verify_receipt_list` is in tree at `27f8df6`; **not** RPC-wired (`receiptsRoot` RPC / `eth_getLogs` still open). `transactionsRoot` hashes in `eth_getBlock*` — **closed** (raw-RLP DeriveSha; empty root or no envelopes → `[]`; lying list → `-32001`). Count/ByIndex **Verified**. Opt-in `eth_getRawTransactionByHash` — **closed** (keccak-bound). `accessList` prefetch — **closed**. Historical `n≤Safe` — **closed**. Keep `SpecId::CANCUN` on revm 19.7 (not Mendel).
