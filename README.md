...
4. **MVP-2** — constrained `eth_call` + best-effort `eth_estimateGas` (proof-backed revm; never proxied). **Fast Finality (BLS) implemented** — vote attestations decoded and their aggregate BLS signature verified against the epoch vote keys; live mainnet finalized lag **2 blocks** vs 106–112 for confirmation depth ([docs/fast-finality.md](docs/fast-finality.md)). The `finalized` tag still resolves to the confirmation-depth Safe head; the ≥24h soak that gates moving it passed 2026-08-24, so that flip is the next implementation step.
